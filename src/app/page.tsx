...
//         <Link href="/emicalc" className="ml-4 bg-purple-600 text-gray-700 px-4 py-2 rounded hover:bg-purple-700">EMI Calc</Link>
//       </div>
...
export default function Home() {
  ...
  return (
    <main className="p-6 bg-gray-50 min-h-screen">
      {/* Header bar with EMI link */}
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-900">AutoGuru – Pi Car Lot</h1>
        <Link
          href="/emicalc"
        ...
        {cars.map((c) => (
          <div key={c._id} className="rounded-xl shadow bg-white p-4 flex flex-col">
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button
                onClick={() =>
                  router.push(`/compare?left=${cars[0]?._id}&right=${c._id}`)
                }
      ...
      {/* Gallery modal */}
      {gallery.length > 0 && (
        <GalleryModal images={gallery} onClose={() => setGallery([])} />
      )}
    </main>
  );
}
